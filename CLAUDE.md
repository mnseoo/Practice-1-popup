...
이 디자인 시스템은 "스피릿 + 룰 + 워크플로우" 구조로 운영된다.

### 스피릿(Spirit)
"왜 이 디자인이 좋은 디자인인가"의 추상 원칙. 새 상황에서도 같은 결정을 내리게 하는 기준.
- 룰 예: "버튼은 아래에 둔다"
- 스피릿 예: "다음 행동으로 시선이 자연스럽게 흐르게 한다"

### What vs How (자동화 경계)
- **What** (디자이너 결정): 무엇을 강조할지, 어떤 톤이 맞는지 → 자동화 X. 항상 사용자에게 물을 것.
- **How** (시스템 처리): 어떻게 강조할지, 어떤 토큰 쓸지 → 자동화 O.
...
`components/`의 button · badge · navigation · text-input · modal · popup 은 **그대로** 사용합니다. 각 컴포넌트마다 `.md`(스펙)과 `.json`(토큰)이 쌍으로 있음.

> **modal vs popup**: 분기 의도(취소/확인)가 의미 있으면 **modal** (Confirm Dialog), 인지·전환 유도가 목적이면 **popup** (Marketing/Notice). 자세한 차이는 `components/popup/popup.md` 의 "Modal과의 차이" 섹션 참고.
...
- 작업 마칠 때: `/회고` → 캡처 + 승격 한 번에
- 결과물: 모든 컴포넌트·팝업·페이지 → 스토리북 업로드. 링크가 최종 산출물.

---

## 🚧 진행 중 메모

작업 중 발견한 패턴 후보. 정리 전 임시 보관.
- 2번 이상 반복 → 위 정식 섹션(핵심 스피릿/룰)으로 승격
- 1번뿐 → 다음 /회고 때 점검
